<template>
    <v-container>
      <v-btn v-if="primer"
      class="primary"
      @click="dialog3 = true">Indicar cantidad de Pieles y kilos entrantes</v-btn>
      <div v-else>
        <h5>Procesos planificados:</h5>
        <v-row v-if="!primer">
          <v-col md="2" cols="4" v-for="item in items_curtido" :key="item.codigo">
            <v-card
              class="mx-auto"
              outlined>
              <v-list-item>
      ...
      >
        <v-card>
          <v-card-title>
            <span class="headline">Pieles y Kilos entrantes para la Formula de curtido</span>
          </v-card-title>
          <v-card-text>
            <v-container>
              <v-row>
  ...
  methods: {
    Guardarprimer() {
      ...
      const option = {
        ...
        url: process.env.VUE_APP_URL_SERVER + '/api/act_curtido',
        method: 'POST',
        headers: {
        ...
        .then((res) => {
          ...
          if (data.cod === '200') {
            const nuevorealizado = {
              ...
              carnaza: 0,
              observacion: '',
              codigo_seguimiento: `${cod}-1-${this.listafalta[0]}`,
            };
      ...
      axios(option)
        .then((res) => {
          ...
          if (data.cod === '200') {
            ...
            this.listafalta = data.data.proceso[0].pcurtido.split(',');
            this.listafalta = this.listafalta.sort();
            this.listafalta.splice(0, this.listaRealizada.length);
            this.dialog = false;
            if (data.data.proceso[0].kilosc === 0
            && data.data.proceso[0].pielesc === 0) {
              this.primer = true;
            }
          }
        })
